...
--- Entry behind slot `index` of a gallery: the grids are in data order.
local function entry_at(kind, index)
	if kind == "weapon" then
		data.load_all()
...
local function slot_index(comp, prefix)
...
local function fill(screen, prefix, kind)
	for _, comp in ipairs(screen.comps) do
		local i = slot_index(comp, prefix)
		if i then
			local entry = entry_at(kind, i)
...
function gallery.prepare(screen_name, screen)
	if screen_name == "Weapons" then
		fill(screen, "Weapon", "weapon")
	elseif screen_name == "Perks" then
		fill(screen, "Perk", "perk")
	end
end
...
local function draw_info(screen, hovered, title, detail, title_color)
...
function gallery.draw(screen_name, screen)
	...
	if screen_name == "Weapons" then
		local i = slot_index(hover, "Weapon")
		local w = i and entry_at("weapon", i)
		if not w then return end
		draw_info(screen, hover, w.name or "?",
	...
	elseif screen_name == "Perks" then
		local i = slot_index(hover, "Perk")
		if not i then return end
		local p = entry_at("perk", i)
		if p then
			draw_info(screen, hover, p.name, p.desc or "", BRASS)
		else
			draw_info(screen, hover, "Not in this port yet",
